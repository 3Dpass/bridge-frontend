...
const BridgeForm = () => {
  ...
  useEffect(() => {
    if (network && !formData.sourceNetwork) {
      console.log('🔍 Pre-selecting current network as source:', network.name);
      setFormData(prev => ({ ...prev, sourceNetwork: network.name }));
    }
  }, [network, formData.sourceNetwork]);
