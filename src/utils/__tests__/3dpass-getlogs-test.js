/**
 * Test eth_getLogs method for 3DPass contract
 * Contract: 0x65101a5889F33E303b3753aa7311161F6C708F27 (P3D Export Bridge on 3DPass)
 */

import { NETWORKS } from '../../config/networks.js';

const CONTRACT_ADDRESS = '0x65101a5889F33E303b3753aa7311161F6C708F27';
const NETWORK_KEY = 'THREEDPASS';

async function test3dpassGetLogs() {
  console.log('🧪 Testing eth_getLogs method for 3DPass contract...');
  console.log(`Contract Address: ${CONTRACT_ADDRESS}`);
  console.log(`Network: ${NETWORK_KEY}`);
  console.log('');

  try {
    // Get 3DPass network configuration
    const threedpassConfig = NETWORKS[NETWORK_KEY];
    if (!threedpassConfig) {
      throw new Error(`Network configuration not found for ${NETWORK_KEY}`);
    }

    console.log('📡 Network Configuration:');
    console.log(`  RPC URL: ${threedpassConfig.rpcUrl}`);
    console.log(`  Chain ID: ${threedpassConfig.id}`);
    console.log(`  Explorer: ${threedpassConfig.explorer}`);
    console.log(`  Is Hybrid: ${threedpassConfig.isHybrid}`);
    console.log(`  Is EVM: ${threedpassConfig.isEVM}`);
    console.log('');

    // First, get the latest block number to use a more recent range
    console.log('🔍 Getting latest block number...');
    const latestBlockResponse = await fetch(threedpassConfig.rpcUrl, {
      method: 'POST',
      headers: {
    ...
    const startBlock = Math.max(0, latestBlock - 10000); // Start from 10,000 blocks ago
    ...
    const response = await fetch(threedpassConfig.rpcUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (result.error) {
      ...
      if (logs.length > 0) {
        ...
        if (logs.length > 0) {
          ...
          const blockNumbers = logs.map(log => parseInt(log.blockNumber, 16));
          const minBlock = Math.min(...blockNumbers);
          const maxBlock = Math.max(...blockNumbers);
          const uniqueBlocks = new Set(blockNumbers).size;
          
          console.log(`  Block Range: ${minBlock} - ${maxBlock}`);
          console.log(`  Total Blocks: ${maxBlock - minBlock + 1}`);
          console.log(`  Blocks with Events: ${uniqueBlocks}`);
          console.log(`  Events per Block: ${(logs.length / uniqueBlocks).toFixed(2)}`);
        }
      } else {
        ...
        console.log('    - The 3DPass network might have different event indexing');
      }
    }

    console.log('');
    console.log('🎯 3DPass Test completed successfully!');

  } catch (error) {
    console.error('❌ 3DPass Test failed:', error.message);
